...
/**
  ...
  */
abstract class BaseDockerSpec(dockerEnv: DockerEnv) extends WordSpec with Matchers with Eventually with BeforeAndAfterAll with GivenWhenThen with eie.io.LowPriorityIOImplicits with ScalaFutures with StrictLogging with BeforeAndAfterEach {
